...
LENA-PJ는 **여행 플래너 RAG 에이전트**를 제공합니다.
...
### 3.2 런타임 에이전트 파이프라인 (LangGraph 3노드)
...
### 3.5 TourAPI 클라이언트 (`travel/tour_api/`)
...
1. **하이브리드 검색 RAG** — Gemini 모델을 임베딩·생성에 공유하고, SQLite(정형 필터)+FAISS(시맨틱)를 병합해 후보를 검색.
...
5. **선행 조건** — 여행 에이전트는 `build_travel_index.py`로 인덱스를 먼저 만들어야 합니다.
